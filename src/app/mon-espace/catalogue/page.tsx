...
type Item = {
  ...
  third_price:string; third_currency:string; is_active:boolean; show_secondary?:boolean; show_third?:boolean;
};
type Category = {
...
const catalogIcons = [
  ...
  { key: "fork", icon: "🍴", label: "Restaurant" },
  { key: "bag", icon: "🛍️", label: "Boutique" },
  { key: "sparkles", icon: "✦", label: "Premium" },
  { key: "heart", icon: "♥", label: "Favori" },
  { key: "star", icon: "★", label: "Étoile" },
  { key: "book", icon: "▤", label: "Catalogue" },
  { key: "coffee", icon: "☕", label: "Café" },
  { key: "pizza", icon: "🍕", label: "Pizza" },
  { key: "cake", icon: "🍰", label: "Pâtisserie" },
  { key: "drink", icon: "🥤", label: "Boisson" },
  { key: "scissors", icon: "✂", label: "Coiffure" },
  { key: "beauty", icon: "💅", label: "Beauté" },
  { key: "fitness", icon: "🏋️", label: "Fitness" },
  { key: "medical", icon: "✚", label: "Santé" },
  { key: "car", icon: "🚗", label: "Auto" },
  { key: "home", icon: "⌂", label: "Maison" },
  { key: "tools", icon: "🧰", label: "Services" },
  { key: "camera", icon: "📷", label: "Photo" },
  { key: "music", icon: "♫", label: "Musique" },
  { key: "gift", icon: "🎁", label: "Cadeau" },
  { key: "ticket", icon: "🎟", label: "Ticket" },
  { key: "calendar", icon: "▣", label: "Réservation" },
];

function getCatalogIcon(key?: string) {
  return catalogIcons.find((item) => item.key === key)?.icon || "▦";
}
...
export default function CataloguePage(){
  ...
  async function load(){
    try{
      ...
      setCategories((cats||[]).map((x:any)=>({...x,image_url:x.image_url||"",items:items.filter(i=>i.category_id===x.id).map(i=>({...i,description_fr:i.description_fr||"",description_en:i.description_en||"",image_url:i.image_url||"",icon:i.icon||"sparkles",price:i.price==null?"":String(i.price),
          ...
          third_currency:i.third_currency||"",show_secondary:i.secondary_price!=null||!!i.secondary_currency,show_third:i.third_price!=null||!!i.third_currency}))})));
    }catch(e:any){setMessage(e.message||"Erreur")}finally{setLoading(false)}
  }

  async function toggleCatalogue(enabled:boolean){
    setCard((current:any)=>({...current,catalog_enabled:enabled}));
    setMessage("");
    try{
      const s=getSupabaseBrowser();
      const {error}=await s.from("cards").update({catalog_enabled:enabled}).eq("id",card.id);
      if(error)throw error;
      setMessage(enabled ? (fr?"Catalogue activé.":"Catalog enabled.") : (fr?"Catalogue désactivé.":"Catalog disabled."));
    }catch(e:any){
      setCard((current:any)=>({...current,catalog_enabled:!enabled}));
  ...
  async function upload(file:File,kind:"catalog"|"category"|"item",ci?:number,ii?:number){
    ...
    try{
      if(kind==="category" && !file.type.startsWith("image/")) throw new Error(fr?"La catégorie accepte uniquement une image.":"Category accepts images only.");
      if(kind==="item" && !(file.type.startsWith("image/") || file.type.startsWith("video/"))) throw new Error(fr?"Choisissez une image ou une vidéo.":"Choose an image or a video.");
      const s=getSupabaseBrowser();
      const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");
      const path=`${card.id}/catalog/${kind}-${Date.now()}-${safe}`;
      const {error}=await s.storage.from("card-assets").upload(path,file,{upsert:true,contentType:file.type||undefined});
      if(error)throw error;
      const {data}=s.storage.from("card-assets").getPublicUrl(path);
      const publicUrl=data.publicUrl;
      if(kind==="catalog")setCard((current:any)=>({...current,catalog_file_url:publicUrl,catalog_file_name:file.name}));
      if(kind==="category"&&ci!==undefined)updateCategory(ci,{image_url:publicUrl,visual_type:"image"});
      if(kind==="item"&&ci!==undefined&&ii!==undefined)updateItem(ci,ii,{image_url:publicUrl,visual_type:"image"});
      setMessage(file.type.startsWith("video/")?(fr?"Vidéo uploadée. Cliquez sur Enregistrer le catalogue.":"Video uploaded. Click Save catalog."):(fr?"Image uploadée. Cliquez sur Enregistrer le catalogue.":"Image uploaded. Click Save catalog."));
  ...
  function addItem(c:number){setCategories(v=>v.map((x,n)=>n!==c?x:{...x,items:[...x.items,{id:`new-${id()}`,title_fr:"",title_en:"",description_fr:"",description_en:"",visual_type:"none",image_url:"",icon:"sparkles",price:"",price_mode:"fixed",currency:card?.catalog_primary_currency||"TND",
        ...
        third_price:"",third_currency:"",is_active:true,show_secondary:false,show_third:false}]}))}
  ...
  async function translateText(text:string,source:"fr"|"en",target:"fr"|"en"){
    const clean=text.trim();
    if(!clean)return "";
    try{
      const r=await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(clean)}&langpair=${source}|${target}`);
      if(!r.ok)throw new Error("translate");
      const data=await r.json();
      return String(data?.responseData?.translatedText||clean);
    }catch{return clean}
  }

  async function autoTranslate(value:string,onDone:(translated:string)=>void){
    if(!value.trim())return;
    setTranslating(true);
    try{onDone(await translateText(value,fr?"fr":"en",fr?"en":"fr"))}finally{setTranslating(false)}
  }

  async function refreshExchangeRate(primary?:string,secondary?:string){
    const from=primary||card?.catalog_primary_currency||"TND";
    const to=secondary||card?.catalog_secondary_currency||"EUR";
    if(from===to)return;
    try{
      ...
      if(Number.isFinite(rate)&&rate>0)setCard((current:any)=>({...current,catalog_exchange_rate:rate,catalog_exchange_rate_updated_at:new Date().toISOString()}));
    }catch{/* garde le dernier taux disponible */}
  }
  ...
  return <main className="page">
    <header className="hero"><div><span>VISITECARD</span><h1>{fr?"Catalogue":"Catalog"}</h1><p>{fr?"Créez votre menu, catalogue ou liste de prestations.":"Create your menu, catalog or services."}</p></div><label className={`switch ${card.catalog_enabled?"on":""}`}><input type="checkbox" checked={!!card.catalog_enabled} onChange={e=>void toggleCatalogue(e.target.checked)}/><i/><b>{card.catalog_enabled?"ON":"OFF"}</b></label></header>

    <section className="panel">
      <div className="panelTitle"><div><small>01</small><h2>{fr?"Bouton public":"Public button"}</h2></div><div className="preview" style={{background:card.catalog_button_color||"#111827",color:card.catalog_button_text_color||"#fff"}}><span>{getCatalogIcon(card.catalog_icon)}</span>{fr ? (card.catalog_label_fr||"Nos services") : (card.catalog_label_en||"Our services")}</div></div>
      <div className="simpleSettings">
        <label>{fr?"Nom du bouton":"Button name"}<input value={fr ? (card.catalog_label_fr||"") : (card.catalog_label_en||"")} onChange={e=>setCard({...card,[fr ? "catalog_label_fr" : "catalog_label_en"]:e.target.value})} onBlur={e=>void autoTranslate(e.target.value,t=>setCard((current:any)=>({...current,[fr?"catalog_label_en":"catalog_label_fr"]:t})))}/><small className="autoHint">FR ⇄ EN {translating?"…":"auto"}</small></label>
        <label>{fr?"Devise":"Currency"}<select value={card.catalog_primary_currency||"TND"} onChange={e=>{const v=e.target.value;setCard({...card,catalog_primary_currency:v});if(card.catalog_auto_convert)void refreshExchangeRate(v,card.catalog_secondary_currency||"EUR")}}>{["TND","EUR","USD","GBP","CAD","AED","SAR","QAR","CHF"].map(x=><option key={x}>{x}</option>)}</select></label>
        <label className="compactToggle"><input type="checkbox" checked={!!card.catalog_auto_convert} onChange={e=>{const checked=e.target.checked;setCard({...card,catalog_auto_convert:checked});if(checked)void refreshExchangeRate(card.catalog_primary_currency||"TND",card.catalog_secondary_currency||"EUR")}}/><span>{fr?"Prix convertis auto":"Auto price conversion"}</span></label>
        {card.catalog_auto_convert?<label>{fr?"Afficher aussi en":"Also show in"}<select value={card.catalog_secondary_currency||"EUR"} onChange={e=>{const v=e.target.value;setCard({...card,catalog_secondary_currency:v,catalog_show_secondary_currency:true});void refreshExchangeRate(card.catalog_primary_currency||"TND",v)}}>{["EUR","USD","TND","GBP","CAD","AED","SAR","QAR","CHF"].filter(x=>x!==card.catalog_primary_currency).map(x=><option key={x}>{x}</option>)}</select></label>:null}
      </div>
      <div className="appearance"><label>{fr?"Fond du bouton":"Button background"}<input type="color" value={card.catalog_button_color||"#111827"} onChange={e=>setCard({...card,catalog_button_color:e.target.value})}/></label><label>{fr?"Texte du bouton":"Button text"}<input type="color" value={card.catalog_button_text_color||"#ffffff"} onChange={e=>setCard({...card,catalog_button_text_color:e.target.value})}/></label><div><b>{fr?"Icône":"Icon"}</b><div className="icons">{catalogIcons.map(item=><button type="button" className={card.catalog_icon===item.key?"active":""} key={item.key} onClick={()=>setCard({...card,catalog_icon:item.key})} title={item.label} aria-label={item.label}><span>{item.icon}</span></button>)}</div></div></div>
    </section>
    ...
    {mode==="manual"?<section className="builder">
      ...
      {categories.map((c,ci)=><article key={c.id} className="category" draggable onDragStart={()=>setDragCat(ci)} onDragOver={e=>e.preventDefault()} onDrop={e=>dropCategory(e,ci)}>
        <div className="catHero" style={c.visual_type==="image"&&c.image_url?{backgroundImage:`linear-gradient(#0005,#0005),url(${c.image_url})`,color:c.text_color}:{background:c.background_color,color:c.text_color}}>
          <span className="grip">⋮⋮</span><div><small>{fr?"CATÉGORIE":"CATEGORY"} {ci+1}</small><h3>{fr ? (c.title_fr||"Nouvelle catégorie") : (c.title_en||"New category")}</h3></div><label className="visible"><input type="checkbox" checked={c.is_active} onChange={e=>updateCategory(ci,{is_active:e.target.checked})}/> {fr?"Visible":"Visible"}</label><button className="trash" onClick={()=>removeCategory(ci)}>×</button>
        </div>
        <div className="catBody">
          <div className="two"><label>{fr?"Nom de la catégorie":"Category name"}<input value={fr ? c.title_fr : c.title_en} onChange={e=>updateCategory(ci,fr ? {title_fr:e.target.value} : {title_en:e.target.value})} onBlur={e=>void autoTranslate(e.target.value,t=>updateCategory(ci,fr?{title_en:t}:{title_fr:t}))}/><small className="autoHint">FR ⇄ EN auto</small></label><div><b>{fr?"Arrière-plan":"Background"}</b><div className="seg"><button className={c.visual_type==="color"?"active":""} onClick={()=>updateCategory(ci,{visual_type:"color"})}>{fr?"Couleur":"Color"}</button><button className={c.visual_type==="image"?"active":""} onClick={()=>updateCategory(ci,{visual_type:"image"})}>{fr?"Image":"Image"}</button></div></div></div>
          ...
          <div className="services">{c.items.map((x,ii)=><div className="service" key={x.id} draggable onDragStart={()=>setDragItem({c:ci,i:ii})} onDragOver={e=>e.preventDefault()} onDrop={e=>dropItem(e,ci,ii)}>
            ...
            <label className="photo">{x.image_url?(isVideoUrl(x.image_url)?<video src={x.image_url} muted playsInline/>:<img src={x.image_url} alt=""/>):<span>＋<small>{fr?"Photo / vidéo":"Photo / video"}</small></span>}<input type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/ogg,video/quicktime" onChange={e=>{const f=e.target.files?.[0];if(f)void upload(f,"item",ci,ii)}}/></label>
            <div className="serviceFields">
              <div className="textFields"><input className="titleInput" placeholder={fr?"Nom du service / produit":"Product / service name"} value={fr ? x.title_fr : x.title_en} onChange={e=>updateItem(ci,ii,fr ? {title_fr:e.target.value} : {title_en:e.target.value})} onBlur={e=>void autoTranslate(e.target.value,t=>updateItem(ci,ii,fr?{title_en:t}:{title_fr:t}))}/><textarea placeholder={fr?"Description":"Description"} value={fr ? x.description_fr : x.description_en} onChange={e=>updateItem(ci,ii,fr ? {description_fr:e.target.value} : {description_en:e.target.value})} onBlur={e=>void autoTranslate(e.target.value,t=>updateItem(ci,ii,fr?{description_en:t}:{description_fr:t}))}/><small className="autoHint">FR ⇄ EN auto</small></div>
              <div className="pricing">
                <select className="priceMode" value={x.price_mode} onChange={e=>updateItem(ci,ii,{price_mode:e.target.value as Item["price_mode"]})}>
                  <option value="fixed">{fr?"Prix fixe":"Fixed price"}</option><option value="from">{fr?"À partir de":"From"}</option><option value="range">{fr?"Fourchette":"Range"}</option><option value="hidden">{fr?"Sans prix":"No price"}</option>
                </select>
                {x.price_mode!=="hidden"?<>
                  <div className="priceLine"><input type="number" min="0" step=".01" placeholder={fr?"Prix":"Price"} value={x.price} onChange={e=>updateItem(ci,ii,{price:e.target.value})}/><select value={x.currency||card.catalog_primary_currency||"TND"} onChange={e=>updateItem(ci,ii,{currency:e.target.value})}>{["TND","EUR","USD","GBP","CAD","AED","SAR","QAR","CHF"].map(v=><option key={v}>{v}</option>)}</select></div>
                  {card.catalog_auto_convert&&x.price?<div className="convertedPill">≈ {convertedPrice(x.price)||"—"} {card.catalog_secondary_currency||"EUR"}</div>:null}
                  {x.show_secondary?<div className="extraPrice"><div className="priceLine"><input type="number" min="0" step=".01" placeholder={fr?"Autre prix":"Other price"} value={x.secondary_price} onChange={e=>updateItem(ci,ii,{secondary_price:e.target.value})}/><select value={x.secondary_currency||x.currency||card.catalog_primary_currency||"TND"} onChange={e=>updateItem(ci,ii,{secondary_currency:e.target.value})}>{["TND","EUR","USD","GBP","CAD","AED","SAR","QAR","CHF"].map(v=><option key={v}>{v}</option>)}</select></div><button type="button" className="removePrice" onClick={()=>updateItem(ci,ii,{secondary_price:"",secondary_currency:"",show_secondary:false})}>×</button></div>:null}
                  {x.show_third?<div className="extraPrice"><div className="priceLine"><input type="number" min="0" step=".01" placeholder={fr?"Autre prix":"Other price"} value={x.third_price} onChange={e=>updateItem(ci,ii,{third_price:e.target.value})}/><select value={x.third_currency||x.currency||card.catalog_primary_currency||"TND"} onChange={e=>updateItem(ci,ii,{third_currency:e.target.value})}>{["TND","EUR","USD","GBP","CAD","AED","SAR","QAR","CHF"].map(v=><option key={v}>{v}</option>)}</select></div><button type="button" className="removePrice" onClick={()=>updateItem(ci,ii,{third_price:"",third_currency:"",show_third:false})}>×</button></div>:null}
                  {!x.show_secondary?<button type="button" className="addPrice" onClick={()=>updateItem(ci,ii,{show_secondary:true,secondary_currency:x.currency||card.catalog_primary_currency||"TND"})}>＋ {fr?"Ajouter un prix":"Add a price"}</button>:!x.show_third?<button type="button" className="addPrice" onClick={()=>updateItem(ci,ii,{show_third:true,third_currency:x.currency||card.catalog_primary_currency||"TND"})}>＋ {fr?"Ajouter un prix":"Add a price"}</button>:null}
                </>:null}
              </div>
            </div>
