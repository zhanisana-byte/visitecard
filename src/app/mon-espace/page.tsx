...
export default function MonEspacePage() {
  ...
  async function saveCard(event?: FormEvent) {
    ...
    try {
      ...
      const baseSlug =
        card.slug ||
        `${slugify(card.full_name) || "carte"}-${user.id.slice(0, 6)}`;
      ...
      setCard((previous) => ({
        ...
        social_links: Array.isArray(data?.social_links)
          ? data.social_links.map((item: any) => ({
              ...
              label: item.label || item.name || networkName(item.type || "website"),
              value: item.value || item.url || "",
            }))
          : cleanSocialLinks,
        custom_links: Array.isArray(data?.custom_links)
          ? data.custom_links.map((item: any) => ({
              id: item.id || uid(),
      ...
      console.error("SAVE CARD ERROR:", saveError);
      setError(
        saveError?.message || "Impossible d'enregistrer votre carte."
      );
    } finally {
      setSaving(false);
    }
