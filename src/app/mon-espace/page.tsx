...
export default function MonEspacePage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      try {
        ...
        const response =
          await fetch(
            `${supabaseUrl}/rest/v1/cards?user_id=eq.${currentUserId}&select=*`,
            {
              headers: {
                apikey:
                  supabaseKey,
  ...
  function readImage(
    ...
    if (
      ...
      setError(
        kind === "cover"
          ? "La couverture doit faire moins de 3,5 Mo."
    ...
    <main className="dashboard">
      <header className="topbar">
        <div className="topbarMain">
          <Link href="/" className="brand" aria-label="VisiteCard"><img src="/logo-visitecard.png.png" alt="VisiteCard" className="brandLogo" /></Link>
          <nav className="desktopNav" aria-label="Navigation principale">
            <button type="button" className="navTop active" onClick={() => router.push("/mon-espace")}>⌂ <span>Accueil</span></button>
            <button type="button" className="navTop" onClick={() => router.push("/mon-espace/statistiques")}>▥ <span>Statistiques</span></button>
            <button type="button" className="navTop" onClick={() => router.push("/mon-espace/avis")}>◯ <span>Avis</span></button>
            <button type="button" className="navTop" onClick={() => publicUrl && window.open(publicUrl,"_blank","noopener,noreferrer")}>◉ <span>Aperçu</span></button>
          </nav>
          <div className="headerTools">
            <div className="languageSwitch"><button type="button" className={card.language === "fr" ? "selected" : ""} onClick={() => updateField("language","fr")}>FR</button><span>|</span><button type="button" className={card.language === "en" ? "selected" : ""} onClick={() => updateField("language","en")}>EN</button></div>
            <button type="button" className="shareIconTop" onClick={sharePublicLink} aria-label="Partager ma carte">↗</button>
            <button type="button" className="menuToggle" onClick={() => setMobileMenuOpen(v => !v)} aria-label="Menu">☰</button>
            <button type="button" className="logout desktopLogout" onClick={logout}>Déconnexion</button>
          </div>
        </div>
        ...
        {mobileMenuOpen ? <div className="mobileMenuPanel"><button type="button" onClick={copyPublicLink}>Copier le lien public</button><button type="button" onClick={sharePublicLink}>Partager ma carte</button><button type="button" className="danger" onClick={logout}>Déconnexion</button></div> : null}
      </header>
      ...
      <style jsx global>{`
        * {
        ...
        .topbar { min-height:78px; padding:0 34px; border-bottom:1px solid #e8e8e6; background:rgba(255,255,255,.97); position:sticky; top:0; z-index:50; backdrop-filter:blur(14px); box-shadow:0 5px 20px rgba(15,23,42,.035); }
        .topbarMain { min-height:78px; display:grid; grid-template-columns:minmax(180px,1fr) auto minmax(260px,1fr); align-items:center; gap:22px; }
        .brand { display:inline-flex; align-items:center; width:fit-content; text-decoration:none; }
        .brandLogo { display:block; width:174px; height:52px; object-fit:contain; object-position:left center; }
        .desktopNav,.mobileNav { display:flex; align-items:center; gap:9px; }
        .navTop { min-height:44px; padding:0 16px; border:1px solid #dedede; border-radius:13px; background:#fff; color:#111; font-size:14px; font-weight:800; cursor:pointer; white-space:nowrap; transition:.18s ease; }
        .navTop:hover { border-color:#ff4f23; color:#ff4f23; transform:translateY(-1px); }
        .navTop.active { background:#ff4f23; border-color:#ff4f23; color:#fff; box-shadow:0 7px 18px rgba(255,79,35,.18); }
        .headerTools { display:flex; align-items:center; justify-content:flex-end; gap:9px; }
        .languageSwitch { display:flex; align-items:center; gap:7px; color:#a0a4aa; font-weight:900; }
        .languageSwitch button { border:0; padding:7px 2px; background:transparent; color:#9ca3af; font-weight:900; cursor:pointer; }
        .languageSwitch button.selected { color:#ff4f23; }
        .shareIconTop,.menuToggle { width:42px; height:42px; border:1px solid #dedede; border-radius:12px; display:grid; place-items:center; background:#fff; color:#111; font-size:19px; font-weight:900; cursor:pointer; }
        .menuToggle { display:none; font-size:22px; }
        .logout { min-height:42px; padding:0 14px; border:0; border-radius:12px; background:#111; color:#fff; font-weight:800; cursor:pointer; }
        .mobileNav,.mobileMenuPanel { display:none; }
        ...
        @media (max-width: 760px) {
          .topbar { padding:0; min-height:auto; }
          .topbarMain { min-height:72px; padding:8px 14px 6px; display:grid; grid-template-columns:minmax(120px,1fr) auto; gap:10px; }
          .brandLogo { width:150px; height:48px; }
          .desktopNav,.desktopLogout { display:none; }
          .headerTools { gap:6px; }
          .languageSwitch { gap:5px; font-size:13px; }
          .shareIconTop,.menuToggle { width:38px; height:38px; border-radius:11px; }
          .menuToggle { display:grid; }
          .mobileNav { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:6px; padding:6px 10px 10px; }
          .mobileNav .navTop { min-width:0; min-height:42px; padding:0 5px; border-radius:12px; font-size:11px; }
          .mobileMenuPanel { display:grid; gap:7px; position:absolute; top:68px; right:12px; width:min(250px,calc(100vw - 24px)); padding:10px; border:1px solid #e7e7e7; border-radius:16px; background:#fff; box-shadow:0 18px 50px rgba(15,23,42,.15); z-index:80; }
          .mobileMenuPanel button { min-height:42px; border:0; border-radius:11px; background:#f7f7f7; color:#111; font-weight:800; text-align:left; padding:0 13px; }
          .mobileMenuPanel .danger { background:#111; color:#fff; }
