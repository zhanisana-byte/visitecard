      ...
      {categories.map((c,ci)=><article key={c.id} className="category" draggable onDragStart={()=>setDragCat(ci)} onDragOver={e=>e.preventDefault()} onDrop={e=>dropCategory(e,ci)}>
        ...
        <div className="catBody">
          ...
          <div className="services">{c.items.map((x,ii)=><div className="service" key={x.id} draggable onDragStart={()=>setDragItem({c:ci,i:ii})} onDragOver={e=>e.preventDefault()} onDrop={e=>dropItem(e,ci,ii)}>
            ...
            <div className="serviceFields"><input className="titleInput" placeholder={fr?"Nom du service / produit (FR)":"Product / service name (EN)"} value={fr ? x.title_fr : x.title_en} onChange={e=>updateItem(ci,ii,fr ? {title_fr:e.target.value} : {title_en:e.target.value})}/><textarea placeholder={fr?"Description (FR)":"Description (EN)"} value={fr ? x.description_fr : x.description_en} onChange={e=>updateItem(ci,ii,fr ? {description_fr:e.target.value} : {description_en:e.target.value})}/><div className="pricing">
...
@media(max-width:850px){.page{padding:8px 14px 88px}.conversionBox{grid-template-columns:1fr}.page>.hero{align-items:flex-start!important;margin-bottom:12px!important}.page>.hero h1{font-size:32px}.page>.hero p{font-size:14px}.modes{grid-template-columns:1fr 1fr}.two,.appearance{grid-template-columns:1fr}.panel{padding:17px;border-radius:18px}.category{border-radius:18px}.builderHead{align-items:flex-start;flex-direction:column}.builderHead .primary{width:100%}.catHero{min-height:72px;padding:14px}.catHero h3{font-size:20px}.catBody{padding:14px}.service{grid-template-columns:auto 64px minmax(0,1fr);padding:10px;gap:9px}.serviceFields{grid-template-columns:1fr;grid-column:3}.pricing{grid-column:2/-1}.serviceActions{grid-column:2/-1;flex-direction:row;justify-content:space-between}.photo{width:64px;height:64px}.panelTitle{align-items:flex-start;flex-direction:column}.preview{width:100%;box-sizing:border-box;justify-content:center}.save{bottom:10px}.appearance{margin-top:10px}}
