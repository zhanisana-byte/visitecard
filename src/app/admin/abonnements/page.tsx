...
type Subscription = {
  ...
  card:CardInfo|null; included_via:CardInfo|null; is_missing?:boolean;
};
...
export default function AdminSubscriptionsPage(){
  ...
  const rows=useMemo(()=>items.filter(s=>{
    ...
    if(status==="missing"&&!s.is_missing)return false;
    if(status!=="all"&&status!=="missing"&&s.status!==status)return false;
  ...
  async function save(){
    ...
    try{
      const payload={
        ...(current.is_missing?{card_id:current.card_id}:{id:current.id}),
        status:form.status,plan_code:form.plan_code,currency:form.currency,
      ...
      const r=await fetch("/api/admin/subscriptions",{method:current.is_missing?"POST":"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});
    ...
    <div className="wrap">
      ...
      <section className="panel">
        <div className="filters">
          ...
          <select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">Tous les statuts</option><option value="missing">À configurer</option>{Object.entries(labels).map(([k,v])=><option key={k} value={k}>{String(v)}</option>)}</select>
